...
export function buildQuestionnairePdf(patient: Patient | undefined, studio: StudioProfile, foods: Food[], filled = false) {
  ...
  const rows: RowInput[] = [
    ...
    ...[
      ...
      ['Abitudini, orari dei pasti, esigenze pratiche', intake?.habits],
      ['Altre informazioni da discutere con il nutrizionista', ''],
    ].map(([title, value]) => [{ content: `${title}\n${value ? clean(value) : '\n________________________________________________________________________\n\n________________________________________________________________________'}`, styles: { minCellHeight: 27 } }]),
