...
export function DietGenerator() {
  const { draft, foods, patients, measurements, setDraft } = useAppStore()
  ...
  try { if (patient) estimatedMaintenance = estimateEnergyNeeds(patient, measurements.filter(m => m.patientId === patient.id)).maintenanceKcal } catch { /* The patient form explains missing inputs. */ }
  const suggestedKcal = patient?.energyProfile?.targetKcal ?? estimatedMaintenance
  return <><button className="button secondary" onClick={() => { setError(''); setReviewed(false); setKcal(suggestedKcal ? String(Math.round(suggestedKcal)) : ''); setOpen(true) }}><Sparkles size={17} />Compila settimana</button>{open && <Modal title="Compila una bozza settimanale" onClose={() => setOpen(false)}><form className="form-stack" onSubmit={e => {
