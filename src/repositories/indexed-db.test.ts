...
describe('repository IndexedDB', () => {
  ...
  it('salva obiettivi calorici e macro soltanto quando le percentuali totalizzano 100', async () => {
    const energyProfile = { activityLevel: 'moderate' as const, goal: 'lose' as const, goalStrategy: 'percentage' as const, goalPercent: 15, calculationMethod: 'cunningham' as const, bodyFatPercent: 24, adjustmentKcal: -200, targetKcal: 2100, macroProfile: 'moderate-carb' as const, macroTargets: { carbsPercent: 45, proteinPercent: 25, fatPercent: 30 } }
    ...
    await expect(repo.patients.save({ ...saved, energyProfile: { ...energyProfile, bodyFatPercent: 80 } })).rejects.toThrow()
  })
